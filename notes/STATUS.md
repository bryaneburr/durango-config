...
## 2025-11-08 — PyPI Release

- Published the first public Durango build to PyPI and verified installation instructions across README, docs, and badges.
- Ran `uv run invoke release` (bump → build → publish → tag) using the existing automation; smoke tests against TestPyPI looked good beforehand.
- Coordinated next steps with dorgy owners to consume the PyPI package via lazy imports and shared CLI helpers.
- Next: track dorgy integration status here and capture any follow-up issues from early adopters.
...
- Next (completed 2025-11-08): fill in remaining release metadata (long description URLs, project homepage) before first upload.
...
- Next session (completed across 2025-02-15 and 2025-11-08): extend documentation, finalize release metadata, and draft publishing workflow.
